...
export async function generate(kind, directory, options) {
  ...
  const config = { kind: kind === 'create-market' ? 'market' : 'source', packageName: options.name, marketId: identity.id, ...(kind === 'create-market' ? { brand, registry: https(options.registry) } : { sourceId, title: options.title ?? 'Team catalog' }), source };
